...
export class ProfitlossComponent implements OnInit {
  ...
  ngOnInit() {
    ...
    this.titleService.setTitle("Profit loss");
    this.data.currentTitle.subscribe(Title => this.Title = Title)
    this.data.changeTitle('Profit & Loss')
  ...
  onGridReady(params) {
    ...
    this.rowData = this.http.get<any>(holder + report, { params: { 'report_type': 'profit_loss' } });
  ...
  teston() {
    ...
    this.rowData = this.http.post(final, this.first_date, { params: { 'report_type': 'profit_loss' } }).toPromise().then((e: any) => {
      if (e.length > 0) {
        this.overlayNoRowsTemplate =
          '<span class="ag-overlay-loading-center">Please wait while your rows are loading</span>';
        return e
